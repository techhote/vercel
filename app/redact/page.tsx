      ...
      <main className="flex-1 py-12">
        <div className="container mx-auto max-w-4xl px-6">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-lg bg-slate-600 flex items-center justify-center mx-auto mb-4">
              <Eye className="h-8 w-8 text-white" />
            </div>
            <h1 className="text-3xl font-bold mb-2">Redact PDF</h1>
            <p className="text-muted-foreground">Permanently remove sensitive information from a PDF</p>
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Upload PDF File</CardTitle>
              <CardDescription>Select a PDF file to redact sensitive information</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-4">
                <Button onClick={handleRedact} disabled={!file || isProcessing} className="flex-1">
                  {isProcessing ? (
                    <>
                ...
                {downloadUrl && (
                  <Button variant="outline" asChild>
                    <a href={downloadUrl} download={`${file?.name.replace(".pdf", "")}-redacted.pdf`}>
