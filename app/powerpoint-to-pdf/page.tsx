...
import { Upload, FileText, Download, Loader2, Presentation } from "lucide-react"
...
export default function PowerPointToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Failed to convert presentation", variant: "destructive" })
    } finally {
      ...
      <main className="flex-1 py-12">
        <div className="container mx-auto max-w-4xl px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">Make PPT and PPTX slideshows easy to view by converting them to PDF</p>
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Upload PowerPoint File</CardTitle>
              <CardDescription>Select a PowerPoint file to convert to PDF format</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="border-2 border-dashed rounded-lg p-8 text-center hover:border-primary/50 transition-colors">
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">PPT, PPTX files only</p>
                </label>
              </div>
              ...
              <div className="flex gap-4">
                ...
                {downloadUrl && (
                  <Button variant="outline" asChild>
                    <a href={downloadUrl} download={`${file?.name.replace(/\.(ppt|pptx)$/, "")}.pdf`}>
